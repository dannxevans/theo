...
class IntentClassifier:
    ...
    def _keyword_match(
        ...
        explicit_planning_patterns = [
            r'\b(plan|schedule|organize|set up)\s+(a|an|my)?\s*(trip|meeting|appointment|activity|event)',
            r'\b(let\'?s|can you|could you|help me|i need to)\s+(plan|schedule|organize)',
            r'\b(plan|schedule)\s+(for|this|that)',
            r'\b(planning|scheduled)\s+(a|an|my)',
            r'\bi need a plan',
        ]

        # Check if user is explicitly requesting planning help
